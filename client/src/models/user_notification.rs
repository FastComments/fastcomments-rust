/*
 * fastcomments
 *
 * No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)
 *
 * The version of the OpenAPI document: 0.0.0
 * 
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct UserNotification {
    #[serde(rename = "_id")]
    pub _id: String,
    #[serde(rename = "tenantId")]
    pub tenant_id: String,
    #[serde(rename = "userId", default, with = "::serde_with::rust::double_option", skip_serializing_if = "Option::is_none")]
    pub user_id: Option<Option<String>>,
    #[serde(rename = "anonUserId", default, with = "::serde_with::rust::double_option", skip_serializing_if = "Option::is_none")]
    pub anon_user_id: Option<Option<String>>,
    #[serde(rename = "urlId")]
    pub url_id: String,
    #[serde(rename = "url")]
    pub url: String,
    #[serde(rename = "pageTitle", default, with = "::serde_with::rust::double_option", skip_serializing_if = "Option::is_none")]
    pub page_title: Option<Option<String>>,
    #[serde(rename = "relatedObjectType")]
    pub related_object_type: f64,
    #[serde(rename = "relatedObjectId")]
    pub related_object_id: String,
    #[serde(rename = "viewed")]
    pub viewed: bool,
    #[serde(rename = "isUnreadMessage")]
    pub is_unread_message: bool,
    #[serde(rename = "sent")]
    pub sent: bool,
    #[serde(rename = "createdAt")]
    pub created_at: String,
    #[serde(rename = "type")]
    pub r#type: models::NotificationType,
    #[serde(rename = "fromCommentId", default, with = "::serde_with::rust::double_option", skip_serializing_if = "Option::is_none")]
    pub from_comment_id: Option<Option<String>>,
    #[serde(rename = "fromVoteId", default, with = "::serde_with::rust::double_option", skip_serializing_if = "Option::is_none")]
    pub from_vote_id: Option<Option<String>>,
    #[serde(rename = "fromUserName", default, with = "::serde_with::rust::double_option", skip_serializing_if = "Option::is_none")]
    pub from_user_name: Option<Option<String>>,
    #[serde(rename = "fromUserId", default, with = "::serde_with::rust::double_option", skip_serializing_if = "Option::is_none")]
    pub from_user_id: Option<Option<String>>,
    #[serde(rename = "fromUserAvatarSrc", default, with = "::serde_with::rust::double_option", skip_serializing_if = "Option::is_none")]
    pub from_user_avatar_src: Option<Option<String>>,
    #[serde(rename = "optedOut")]
    pub opted_out: bool,
}

impl UserNotification {
    pub fn new(_id: String, tenant_id: String, url_id: String, url: String, related_object_type: f64, related_object_id: String, viewed: bool, is_unread_message: bool, sent: bool, created_at: String, r#type: models::NotificationType, opted_out: bool) -> UserNotification {
        UserNotification {
            _id,
            tenant_id,
            user_id: None,
            anon_user_id: None,
            url_id,
            url,
            page_title: None,
            related_object_type,
            related_object_id,
            viewed,
            is_unread_message,
            sent,
            created_at,
            r#type,
            from_comment_id: None,
            from_vote_id: None,
            from_user_name: None,
            from_user_id: None,
            from_user_avatar_src: None,
            opted_out,
        }
    }
}

